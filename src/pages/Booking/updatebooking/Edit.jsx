...
const Edit = () => {
  ...
  const submitForm = async (e) => {
    ...
    await axios.put(`http://localhost:7000/api/update/${id}`, booking)
      .then((response) => {
    ...
    <div className='addBooking'>
      ...
      <form className='addBookingForm' onSubmit={submitForm}>
        ...
        <div className='inputGroup'>
          ...
          <select name="roomid" id="roomid" onChange={inputChangeHandler} value={booking.roomid}>
        ...
        <div className='inputGroup'>
          ...
          <input type="text" onChange={inputChangeHandler} name="firstname" id="firstname" value={booking.firstname} placeholder='First name' />
        </div>
        <div className='inputGroup'>
          <label htmlFor="lastname">Last Name</label>
          <input type="text" onChange={inputChangeHandler} name="lastname" id="lastname" value={booking.lastname} placeholder='Last name' />
        </div>
        <div className='inputGroup'>
          <label htmlFor="address">Address</label>
          <input type="text" onChange={inputChangeHandler} name="address" id="address" value={booking.address} placeholder='Address' />
        </div>
        <div className='inputGroup'>
          <label htmlFor="phone">Phone Number</label>
          <input type="text" onChange={inputChangeHandler} name="phone" id="phone" value={booking.phone} placeholder='Phone Number' />
        </div>
        <div className='inputGroup'>
          <label htmlFor="email">Email</label>
          <input type="email" onChange={inputChangeHandler} name="email" id="email" value={booking.email} placeholder='Email address' />
        </div>
        <div className='inputGroup'>
          <label htmlFor="date">Date of Booking</label>
          <input type="date" onChange={inputChangeHandler} name="date" id="date" value={booking.date ? formatDate(booking.date) : ''} />
        </div>
        <div className='inputGroup'>
          <label>Room Type</label>
          <div>
            <input type="radio" id="deluxe" name="roomtype" value="deluxe" onChange={inputChangeHandler} checked={booking.roomtype === "deluxe"} />
            <label className='roomt' htmlFor="deluxe">Deluxe</label>
          </div>
          <div>
            <input type="radio" id="non_ac" name="roomtype" value="non_ac" onChange={inputChangeHandler} checked={booking.roomtype === "non_ac"} />
            <label className='roomt' htmlFor="non_ac">Non-AC</label>
          </div>
        </div>
        <div className='inputGroup'>
